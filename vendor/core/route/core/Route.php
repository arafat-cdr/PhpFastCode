...
class Route {
		...
		if (array_key_exists("middleware", $group)) {
			...
			foreach ($group["middleware"] as $middleware_class_method) {
				...
				if (is_callable($class_method[0], $class_method[1])) {
					...
					print("<p style='color:red;'>Middleware Static Method Is not Callable...</p>");
					error_trace();
				}
	...
	public static function route_error($error) {
		print("<p style='color:red;'>Your Request is Not valid ! Expecting a {$error} Method</p>");
		error_trace();
	}
	...
	private static function handle_action($action) {
		...
		if (is_callable(array($class_object, $class_method[1]))) {
			...
			print("<p style='color:red;'>Action Error ! Class --> {$class_method[0]} or Method --> {$class_method[1]} Is not Exist </p>");
			error_trace();
		}
	}
